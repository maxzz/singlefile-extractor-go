...
func rewriteCSSExtractDataURLs(
	...
	if len(varDefs) > 0 {
		...
		// Extract supported data: assets (images/fonts) into files under "assets/" next to
		// the vars CSS and rewrite url(data:...) tokens to url("assets/...").
		assetsDir := filepath.Join(filepath.Dir(varsCSSPath), "assets")
		keyToAssetRef := map[string]string{}
		hashToName := map[string]string{}
		writtenNames := map[string]bool{}

		rewriteValueToAssets := func(value string) (string, error) {
			hits := iterURLTokens(value)
			if len(hits) == 0 {
				return value, nil
			}
			...
			for _, h := range hits {
				...
				parsed, err := parseDataURL(d.key)
				if err != nil || !isSupportedDataAssetMediaType(parsed.MediaType) {
					b.WriteString(h.token)
					last = h.end
					continue
				}
		...
		for i := range ordered {
			newVal, err := rewriteValueToAssets(ordered[i].value)
			if err != nil {
				return extractDataURLsResult{}, err
			}
			ordered[i].value = newVal
		}
